...
public class CalculoDescuento {
    ...
    public static void pagar(List<Double> carrito, Scanner sc) {
        ...
        System.out.println("Total con IVA: $" + String.format("%.2f", totalConImpuesto));
        System.out.print("¿Desea aplicar un descuento personalizado? (s/n): ");
        String respuesta = sc.nextLine();
        double descuento;
        if (respuesta.equalsIgnoreCase("s")) {
            System.out.print("Ingrese el porcentaje de descuento (ej. 15 para 15%): ");
            try {
                double porcentaje = Double.parseDouble(sc.nextLine());
                descuento = calcularDescuento(totalConImpuesto, porcentaje);
                System.out.println("Descuento aplicado (" + porcentaje + "%): $" + String.format("%.2f", descuento));
            } catch (NumberFormatException e) {
                System.out.println("Porcentaje no válido. Se aplicará 10% por defecto.");
                descuento = calcularDescuento(totalConImpuesto);
                System.out.println("Descuento aplicado (10% por defecto): $" + String.format("%.2f", descuento));
            }
        } else {
            descuento = calcularDescuento(totalConImpuesto);
            System.out.println("Descuento aplicado (10% por defecto): $" + String.format("%.2f", descuento));
        ...
        System.out.println("Monto final a pagar: $" + String.format("%.2f", totalFinal));
        System.out.print("Ingrese su correo para recibir el comprobante: ");
        String correo = sc.nextLine();
        confirmarCompra(correo, totalFinal);
        carrito.clear();
    }
